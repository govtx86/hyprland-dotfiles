...
hl.device({
    name = "mosart-semi.-2.4g-keyboard-mouse-1",
    sensitivity = -0.5
})
hl.device({
...
hl.bind(mainMod .. " + SUPER_L",
    hl.dsp.exec_cmd([[pkill rofi || rofi -show drun -run-shell-command 'zsh -ic "{cmd} && read"']]), { release = true })
...
-- Screenshot
hl.bind("CONTROL + SHIFT + Print",
    hl.dsp.exec_cmd([[grim  -g "$(slurp -d)" "$HOME/Pictures/Screenshots/$(date +"%d-%m-%Y-%H%S").png"]]))
hl.bind("SHIFT + Print", hl.dsp.exec_cmd([[grim -g "$(slurp -d)" - | wl-copy]]))
hl.bind("CONTROL + Print", hl.dsp.exec_cmd([[grim "$HOME/Pictures/Screenshots/$(date +"%d-%m-%Y-%H%S").png"]]))
...
hl.bind(mainMod .. " + O", hl.dsp.workspace.toggle_special("specialwp1"))
hl.bind(mainMod .. " + SHIFT + O", hl.dsp.window.move({ workspace = "special:specialwp1" }))
